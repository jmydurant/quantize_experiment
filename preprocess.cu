...
#define TLLM_CHECK_WITH_INFO(expr, ...)                                        \
  do {                                                                         \
    if (!(expr)) {                                                             \
      printf(__VA_ARGS__);                                                     \
    }                                                                          \
...
{
    int device{-1};
    TLLM_CHECK_WITH_INFO(cudaGetDevice(&device) == cudaSuccess, "error for cudaGetDevice.\n");
    int sm_major = 0;
    int sm_minor = 0;
    TLLM_CHECK_WITH_INFO(cudaDeviceGetAttribute(&sm_major, cudaDevAttrComputeCapabilityMajor, device) == cudaSuccess, "error for get cudaDevAttrComputeCapabilityMajor");
    TLLM_CHECK_WITH_INFO(cudaDeviceGetAttribute(&sm_minor, cudaDevAttrComputeCapabilityMinor, device) == cudaSuccess, "error for get cudaDevAttrComputeCapabilityMinor");
...
LayoutDetails getLayoutDetailsForArch(QuantType quant_type)
{
    ...
    case QuantType::W4_AFP8:
        ...
        break;
    default: printf("Unsupported quantization type");
    }
    return details;
}
...
std::vector<int> get_permutation_map(QuantType quant_type)
{
    ...
    {
        printf("Invalid quantization type for LDSM permutation");
        return {};
    }
}
